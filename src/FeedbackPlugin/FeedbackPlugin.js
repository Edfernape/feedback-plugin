...
export default class FeedbackPlugin extends React.Component{
    constructor(props){
        ...
        this.state = {
            ...
            openerPlacement: this.checkOpenerPlacement(this.props.openerPlacement),
    ...
    checkFormType(formIsPopup, showForm){
        if(showForm === null){
            ...
            if(formIsPopup === true){ // popup form if showForm is true
                if(showForm === true){
                    popupWindow("http://localhost:8081", "", window, 400, 400);
                }
            }else{ // render form depending on showForm true or false
                if(showForm === true){
                    return(
                        <FeedbackForm
                        theme = {this.state.theme}
